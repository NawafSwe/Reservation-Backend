...
describe('testing get table by id functionality', () => {
    let agent, app;
    const url = '/tables';
    ...
    /**
    * @description getting table successfully
    */
    test('getting table by id successfully, returning 200 STATUS', async () => {
        ...
        expect(getTablesResponse.body.data).toHaveProperty('table');
        expect(getTablesResponse.body.data.table).toHaveProperty('id');
        return getTablesResponse;

    });

    /**
    * @description failed getting table due unauthenticated error and invalid id
    */
    test('failed getting table, returning 401 STATUS', async () => {
        return await agent.get(`${url}/c2fe86e0-78b6-4623-befa-4bdf65091169`).expect(HttpStatus.UNAUTHORIZED.code);
    });



    /**
    * @description failed getting table due authority resection where a normal employee trying to consume a resource that only a user with authority of admin can access it  and invalid ID
    */
    test('failed getting tables, returning 401 STATUS', async () => {
        return await agent.get(`${url}/c2fe86e0-78b6-4623-befa-4bdf65091169`)
