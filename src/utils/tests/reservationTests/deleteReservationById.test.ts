...
describe('testing delete reservation by id functionality', () => {
    let agent, app;
    ...
    /**
    * @description deleting reservation by id successfully
    */
    test('deleting reservation by id successfully, returning 200 STATUS', async () => {
        ...
        await agent
            .delete(`${url}/${postingReservation.body.data.reservation.id}`)
            .set('auth', userAuth.token)
    ...
    /**
    * @description failed deleting reservation by id due unauthenticated error
    */
    test('failed deleting reservation by id, returning 401 STATUS', async () => {
        return await agent.delete(`${url}/342o2042-e3-403i32-edj`).expect(HttpStatus.UNAUTHORIZED.code);
    });
    ...
    */
    test('failed deleting reservation by id, returning 401 STATUS', async () => {
        return await agent.delete(`${url}/bce00b8d-430f-4cbe-afdd-54d5ffc10228`)
            .set('auth', userAuth.token)
